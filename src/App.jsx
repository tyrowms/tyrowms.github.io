...
import {
  ...
  AiBrain04Icon, CloudDownloadIcon,
...
export default function App(){
  ...
  const [fcstAnaTrader,setFcstAnaTrader]=useState([]);  // ana trader multi-select
  const [fcstAnaTraderList,setFcstAnaTraderList]=useState([]);  // ana trader option list
  ...
  const loadFcstTraderList=useCallback(async()=>{
    ...
    try{
      const cached=localStorage.getItem('tyrowms_fcst_traders_v3');
      if(cached){
        const p=JSON.parse(cached);
        if(p&&p.fetchedAt&&Date.now()-p.fetchedAt<86400000&&Array.isArray(p.list)&&Array.isArray(p.anaList)){
          setFcstTraderList(p.list);setFcstAnaTraderList(p.anaList);
          // Directory de cache'le (filtreleme için)
    ...
    try{
      ...
      const directory=await fetchTraderDirectory(token);  // aktif + geçerli olanlar zaten filtrelenmiş
      window.__fcstTraderDirectory=directory;  // trader-anatrader bağlantısı için
      const isTrdPrefix=c=>/^(TRD|DNM)[-_A-Z0-9]*$/.test(String(c||'').toUpperCase());
      // Trader listesi (TRD/DNM prefix'li, kod : isim format)
      const list=directory
        .filter(t=>isTrdPrefix(t.traderid))
        .map(t=>({code:t.traderid,name:t.name,maintraderid:t.maintraderid,label:t.name?`${t.traderid} : ${t.name}`:t.traderid}))
        .sort((a,b)=>a.label.localeCompare(b.label,'tr'));
      // Ana trader listesi: distinct maintraderid → name lookup directory'den
      const dirByCode=new Map(directory.map(t=>[t.traderid,t.name]));
      const seenMain=new Set();
      const anaList=[];
      for(const t of directory){
        const mid=t.maintraderid;
        if(!mid||!isTrdPrefix(mid)||seenMain.has(mid))continue;
        seenMain.add(mid);
      ...
      try{localStorage.setItem('tyrowms_fcst_traders_v3',JSON.stringify({fetchedAt:Date.now(),list,anaList,directory}));}catch(_){}
    }catch(e){console.warn('Trader list yüklenemedi:',e);}
    finally{setFcstTraderListLoading(false);}
  },[msalAccount,fcstTraderList.length,fcstAnaTraderList.length,fcstTraderListLoading]);
  ...
  },[fcstTrader,fcstAnaTrader,fcstHorizon,msalAccount,fcstLoading]);
    ...
    <div style={{display:'flex',height:'100vh',fontFamily:$.f,background:$.bg,color:$.t1,overflow:'hidden',WebkitFontSmoothing:'antialiased',fontSize:13}}>
      ...
      <div className={mob?'mob-sb':''} onMouseEnter={()=>{if(!mob&&!sbPinned){clearTimeout(sbTimerRef.current);setSbHov(true);}}} onMouseLeave={()=>{if(!mob&&!sbPinned){sbTimerRef.current=setTimeout(()=>setSbHov(false),250);}}} style={{width:sbExpanded?250:60,minWidth:sbExpanded?250:60,background:'rgba(255,255,255,.95)',backdropFilter:'blur(24px) saturate(180%)',WebkitBackdropFilter:'blur(24px) saturate(180%)',display:mob?'none':'flex',flexDirection:'column',flexShrink:0,borderRadius:18,border:'1px solid rgba(226,231,238,.35)',margin:'10px 0 10px 10px',transition:'width .25s cubic-bezier(.4,0,.2,1), min-width .25s cubic-bezier(.4,0,.2,1)',overflow:'hidden',boxShadow:'0 2px 12px rgba(0,0,0,.04)'}}>
        ...
        <div style={{padding:sbExpanded?'0 12px':'0 8px',flex:1,overflowY:'auto'}}>
          ...
          {sbExpanded&&<div style={{padding:'14px 8px 6px',fontSize:9,fontWeight:700,letterSpacing:1.8,textTransform:'uppercase',color:$.t3,opacity:.45}}>{'Raporlama'}</div>}
          {!sbExpanded&&<div style={{margin:'10px 8px',borderTop:'1px solid rgba(226,231,238,.4)'}}/>}
          {[{id:'rep',icon:FileBarChart,label:'Kırılım Raporu'},{id:'sto',icon:Package,label:'Stok Raporu'}].map(p=>{const isA=pg===p.id;return(
            <div key={p.id} className="sbn" title={!sbExpanded?p.label:undefined} onClick={()=>{setPg(p.id);setSel(null);setDrillFac(null);setDrillWh(null);setAnaDetail(null);setYonDetail(null);setEmSel(null);setEmDrillFac(null);setEmDrillWh(null);setEmDrillL2(null);setSbOpen(false);}} style={{display:'flex',alignItems:'center',gap:10,padding:sbExpanded?'8px 11px':'8px',margin:'1px 0',borderRadius:8,color:isA?$.ac:$.t2,cursor:'pointer',fontSize:12.5,fontWeight:isA?600:500,background:isA?'rgba(13,110,79,.07)':'transparent',position:'relative',transition:'all .2s ease',justifyContent:sbExpanded?'flex-start':'center'}}>
              {isA&&sbExpanded&&<div style={{position:'absolute',left:-12,top:'50%',transform:'translateY(-50%)',width:3,height:18,background:$.ac,borderRadius:'0 3px 3px 0'}}/>}
              <p.icon size={sbExpanded?16:18} strokeWidth={isA?2.2:1.8}/>{sbExpanded&&p.label}
            </div>);})}
          {sbExpanded&&<div style={{padding:'14px 8px 6px',fontSize:9,fontWeight:700,letterSpacing:1.8,textTransform:'uppercase',color:$.t3,opacity:.45}}>{'Simülasyon'}</div>}
          {!sbExpanded&&<div style={{margin:'10px 8px',borderTop:'1px solid rgba(226,231,238,.4)'}}/>}
          {[{id:'fcst',hugeicon:AiBrain04Icon,label:'Satış Tahmini'}].map(p=>{const isA=pg===p.id;return(
            <div key={p.id} className="sbn" title={!sbExpanded?p.label:undefined} onClick={()=>{setPg(p.id);setSel(null);setDrillFac(null);setDrillWh(null);setAnaDetail(null);setYonDetail(null);setEmSel(null);setEmDrillFac(null);setEmDrillWh(null);setEmDrillL2(null);setSbOpen(false);}} style={{display:'flex',alignItems:'center',gap:10,padding:sbExpanded?'8px 11px':'8px',margin:'1px 0',borderRadius:8,color:isA?$.ac:$.t2,cursor:'pointer',fontSize:12.5,fontWeight:isA?600:500,background:isA?'rgba(13,110,79,.07)':'transparent',position:'relative',transition:'all .2s ease',justifyContent:sbExpanded?'flex-start':'center'}}>
              {isA&&sbExpanded&&<div style={{position:'absolute',left:-12,top:'50%',transform:'translateY(-50%)',width:3,height:18,background:$.ac,borderRadius:'0 3px 3px 0'}}/>}
          ...
          {sbExpanded&&<div style={{padding:'14px 8px 6px',fontSize:9,fontWeight:700,letterSpacing:1.8,textTransform:'uppercase',color:$.t3,opacity:.45}}>{'Veri'}</div>}
          {!sbExpanded&&<div style={{margin:'10px 8px',borderTop:'1px solid rgba(226,231,238,.4)'}}/>}
          {[{id:'raw',icon:Database,label:'ERP Veriler'},{id:'erp',hugeicon:CloudDownloadIcon,label:'Ham Veriler'}].map(p=>{const isA=pg===p.id;return(
            <div key={p.id} className="sbn" title={!sbExpanded?p.label:undefined} onClick={()=>{setPg(p.id);setSel(null);setDrillFac(null);setDrillWh(null);setAnaDetail(null);setYonDetail(null);setEmSel(null);setEmDrillFac(null);setEmDrillWh(null);setEmDrillL2(null);setSbOpen(false);}} style={{display:'flex',alignItems:'center',gap:10,padding:sbExpanded?'8px 11px':'8px',margin:'1px 0',borderRadius:8,color:isA?$.ac:$.t2,cursor:'pointer',fontSize:12.5,fontWeight:isA?600:500,background:isA?'rgba(13,110,79,.07)':'transparent',position:'relative',transition:'all .2s ease',justifyContent:sbExpanded?'flex-start':'center'}}>
              {isA&&sbExpanded&&<div style={{position:'absolute',left:-12,top:'50%',transform:'translateY(-50%)',width:3,height:18,background:$.ac,borderRadius:'0 3px 3px 0'}}/>}
              {p.hugeicon?<HugeiconsIcon icon={p.hugeicon} size={sbExpanded?16:18} strokeWidth={isA?2.2:1.8}/>:<p.icon size={sbExpanded?16:18} strokeWidth={isA?2.2:1.8}/>}{sbExpanded&&p.label}
              ...
              return(
                <div>
                    ...
                    <div style={{padding:'18px 20px',display:'flex',alignItems:'flex-end',gap:14,flexWrap:'wrap'}}>
                      ...
                      <div style={{minWidth:240,flex:'1 1 240px'}}>
                        ...
                        <SearchableSelect
                          multi
                          value={fcstAnaTrader}
                          onChange={v=>{setFcstAnaTrader(v);setFcstTrader([]);}}
                          items={fcstAnaTraderList.map(t=>({value:t.code,label:t.label,sub:t.code}))}
                          placeholder={fcstTraderListLoading?'Yükleniyor...':'Ana trader seçin (örn. TRD-DNM)...'}
                      ...
                      <div style={{minWidth:280,flex:'1 1 280px'}}>
                        ...
                        <SearchableSelect
                          ...
                          items={(fcstAnaTrader.length>0?fcstTraderList.filter(t=>fcstAnaTrader.includes(t.maintraderid)):fcstTraderList).map(t=>({value:t.code,label:t.label,sub:t.code}))}
                          placeholder={fcstTraderListLoading?'Yükleniyor...':fcstAnaTrader.length>0?`${fcstAnaTraderList.find(a=>a.code===fcstAnaTrader[0])?.label?.split(':')[0]?.trim()||'Ana'} altındaki tüm traderlar (boş = hepsi)`:'Bir veya birden fazla trader seçin (TRD- / DNM-)...'}
                          disabled={fcstTraderListLoading}
                  ...
                  {fcstLoading&&(()=>{
                    ...
                    const selectedAnaTraders=Array.isArray(fcstAnaTrader)?fcstAnaTrader:[];
                    const useAnaScope=selectedTraders.length===0&&selectedAnaTraders.length>0;
                    const scopeCodes=useAnaScope?selectedAnaTraders:selectedTraders;
                    const scopeList=useAnaScope?fcstAnaTraderList:fcstTraderList;
                      ...
                      <div style={{background:'linear-gradient(135deg,rgba(45,212,160,.04),rgba(59,130,246,.04),rgba(139,92,246,.04))',border:'1px solid '+$.bdL,borderRadius:$.rL,padding:'30px 28px',position:'relative',overflow:'hidden'}}>
                        ...
                        <div style={{position:'relative',display:'flex',flexDirection:'column',alignItems:'center',marginBottom:20}}>
                          ...
                          <div style={{fontSize:12,color:$.t3,fontWeight:500}}>
                            <strong style={{color:$.t2}}>{traderInfo?.name||(useAnaScope?`${selectedAnaTraders.length} ana trader (alt traderlar dahil)`:selectedTraders.length>1?`${selectedTraders.length} trader birleşik`:scopeCodes[0]||'-')}</strong> için <strong style={{color:$.t2}}>{fcstHorizon} ay</strong> ileri tahmin oluşturuluyor
                          </div>
                        </div>
                        {/* Steps */}
